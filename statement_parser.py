...
def _widen(layout: PageLayout) -> None:
    ...
    for i, c in enumerate(cols):
        c.x0 = starts[i]
        c.x1 = starts[i + 1] if i + 1 < len(cols) else 10_000.0
...
def is_noise(text: str) -> bool:
    t = text.strip()
    return not t or bool(NOISE_RE.match(t))
        ...
        with pdfplumber.open(file, password=password or "") as pdf:
            ...
            if self.opt.default_year is None:
                ...
                if year:
                    ...
                    self.log.append(f"Statement year: {year} (for dates printed without one)")

            for pno, page in enumerate(pdf.pages, start=1):
                words = page.extract_words(use_text_flow=False, keep_blank_chars=False)
                if not words:
                    self.log.append(f"Page {pno}: no text layer (scanned page?) - skipped")
                    continue
                ...
                if layout.is_usable() and len(layout.columns) >= self.opt.min_columns_confidence:
                    ...
                    # a later page of the same statement: reuse the header we already
                    ...
                    got = self._parse_blocks(lines, pno)
                    mode = "block mode (Date:/Description:/Amount: layout)"
                else:
                    got = self._parse_fallback(lines, pno)
                    mode = "fallback mode (no header row found)"
                self.log.append(f"Page {pno}: {mode} -> {len(got)} rows")
                rows.extend(got)
